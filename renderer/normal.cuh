...
public:
	normal(): _map(false) {}
	normal(const char* filename,int Width,int Height):width(Width),height(Height),_map(true)
	{
...
#define IMPORT_NORMAL_MAP(name,filename,w,h) normal* name;cudaMalloc(&name,sizeof(normal));cudaMemcpy(name,new normal(filename,w,h),sizeof(normal),cudaMemcpyHostToDevice);
